...
export class CreateTemplateUseCase {
	...
	async execute(input: CreateTemplateInput): Promise<CreateTemplateOutput> {
		...
		const userIntegration = await this.userNotionIntegrationRepository.findById(
		...
		const decryptedNotionToken = await this.encryptionService.decrypt(
		...
		if (!schema) {
			throw new Error(
				`Notion database with ID ${input.notionDatabaseId} not found or schema is inaccessible.`,
			);
		}
