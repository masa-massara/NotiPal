...
export class UpdateTemplateUseCase {
	...
	async execute(input: UpdateTemplateInput): Promise<UpdateTemplateOutput> {
		...
		const getDecryptedToken = async (
			...
			if (!integration)
				throw new Error(
					`User Notion Integration with ID ${integrationId} not found or access denied.`,
				);
		...
		const newNotionDatabaseId =
			input.notionDatabaseId !== undefined
				? input.notionDatabaseId
				: existingTemplate.notionDatabaseId;
		let notionTokenForValidation: string | null = null;

		if (input.userNotionIntegrationId !== undefined) {
			// If userNotionIntegrationId is explicitly in input
			if (
				existingTemplate.userNotionIntegrationId !==
				...
				if (input.userNotionIntegrationId === null) {
					...
					// Changing to a new integration ID
					notionTokenForValidation = await getDecryptedToken(
						input.userNotionIntegrationId,
						input.userId,
					);
					if (!notionTokenForValidation) {
						...
						throw new Error(
							`Failed to retrieve token for User Notion Integration ID ${input.userNotionIntegrationId}.`,
						);
					}
				}
				existingTemplate.userNotionIntegrationId =
				...
				// Integration ID is the same, but maybe notionDatabaseId is changing
				notionTokenForValidation = await getDecryptedToken(
					existingTemplate.userNotionIntegrationId,
					input.userId,
				);
		...
		if (
			...
			const schema = await this.notionApiService.getDatabaseSchema(
				newNotionDatabaseId,
				notionTokenForValidation,
			);
			if (!schema) {
