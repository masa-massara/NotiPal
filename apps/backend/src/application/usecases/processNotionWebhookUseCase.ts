	...
	return async (input: ProcessNotionWebhookInput): Promise<void> => {
		...
		for (const template of templates) {
			...
			try {
				...
				console.error(
					`ProcessNotionWebhookUseCase: Error fetching UserNotionIntegration ID ${template.userNotionIntegrationId} for Template ID ${template.id} (User: ${template.userId}):`,
					repoError,
				);
			...
			if (databaseSchema === null) {
				// Only attempt to fetch schema if not already fetched
				console.log(
